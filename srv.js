  ...
  try {
    ...
    function authenticate(usern, sessID) {
      if (usern in dat.collections.users) {
        if ((dat.collections.sessions[usern] === sessID)) {
    ...
    //}
    //dat.collections.sessions = [];

    function timeAgo(dateObj, currentDateObj) {
      const date = Date.UTC(dateObj.year, dateObj.month - 1, dateObj.day, dateObj.hour, dateObj.minute);
      const currentDate = Date.UTC(currentDateObj.year, currentDateObj.month - 1, currentDateObj.day, currentDateObj.hour, currentDateObj.minute);
    ...
    function makeID(user) {
      let id = '';
      let takenIds = Object.values(dat.collections.sessions);
      ...
      dat.collections.sessions[user] = id;
      return id
    }

    const passwordRequests = ['requestSession']; // A list of request types that require a password instead of a session ID
    const noAuthRequests = ['cr_user']; // A list of request types that do not require authentication
    ...
    function processRequest(str) {
      try {
        ...
        if (!l) {
          ...
          if (noAuthRequests.includes(parsed.type)) {
            if (parsed.type === 'cr_user') {
              if (parsed.name in dat.collections.users) {
                ...
                if (checkChars(parsed.name)) {
                  if (parsed.name.length > 2) {
                    if (parsed.name.length < 16) {
                      dat.collections.users[parsed.name] = { 'key': sha256(parsed.key), 'avatar': [9, 0, 0, null, null, null, 8, [], null, 0], 'bio': 'This user has not yet created a description.', 'roles': [], 'notifs': [], 'unread': { notifications: 0, rooms: {} }, 'requests': [], 'joindate': getMDY(), 'blocked': [], 'consent': POLICY_VERSIONS, 'settings': 'DEFAULT', lastOnline: getMDY()};
                      notify(parsed.name, `Welcome to LibreRooms, ${parsed.name}! If you need help, you can see our guide at https://librerooms.org/guide/. If you want to suggest a change or report an issue or bug, please share feedback with the developer using the report menu.`);
                      dat.collections.rooms["Main room"].members.push(parsed.name);
                      dat.collections.rooms["updates"].members.push(parsed.name);
                      console.log(`Account '${parsed.name}' has been created.`);
                      sysMessage(`@${parsed.name} has joined LibreRooms for the first time!`, 'Main room')
                      return true;
                    } else {
                      return 'longuser';
                    }
          ...
          } else if (passwordRequests.includes(parsed.type)) {
            ...
            if (authenticate(parsed.user, parsed.sessID)) {
              if (parsed.type === 'endSession') {
                delete dat.collections.sessions[parsed.user];
                console.log(`Session for @${parsed.user} has been closed.`);
                return true
              };
              ...
              if (parsed.type === 'getprofile') {
                console.log(`Client request of profile "${parsed.targuser}"`);
                let user = dat.collections.users[parsed.targuser];
                ...
                try {
                  return {
                    ...
                    'avatar': user.avatar,
                    'roles': user.roles,
                    'joindate': timeAgo(user.joindate, getMDY()),
                    'friendcount': getFriends(parsed.targuser).length,
                    'lastOnline': timeAgo(user.lastOnline, getMDY())
                  };
                } catch {
                  return 'NULL';
                }
